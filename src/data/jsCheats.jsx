export const jsCheatsData = [
 {
  title: "ЩО ТАКЕ JS", 
  description: "Основи JavaScript", 
  code: "JavaScript (JS) — це високорівнева мова програмування, що робить веб-сторінки інтерактивними та динамічними. Вона дозволяє змінювати HTML-структуру та CSS-стилі сторінки 'на льоту' (DOM-маніпуляції), реагувати на дії користувача (події), обмінюватися даними з сервером без перезавантаження сторінки (AJAX/Fetch), створювати анімації та складну логіку. Працює як у браузері (клієнт), так і на сервері (Node.js), і є однією з трьох основних технологій веб-розробки разом з HTML та CSS.", 
  preview: <div></div>, 
},
  {
    title: "Animation",
...
myEmitter.emit('myEvent', 'деякі дані'); // Виведе 'Подія сталася! деякі дані'`,
  ...
  .catch(error => {
    ...
    preview: (
      <div>
        <p>
          API <code>Fetch</code> надає сучасний та гнучкий спосіб здійснення
          асинхронних HTTP-запитів у JavaScript (як у браузері, так і в
          Node.js). Він базується на Promise, що полегшує обробку асинхронних
          операцій.
        </p>
        <p className="mt-4">
          Функція <code>fetch()</code> приймає URL ресурсу, який потрібно
          завантажити, та необов'язковий об'єкт параметрів (для налаштування
          методу запиту, заголовків, тіла тощо). Вона повертає Promise, який
          розв'язується з об'єктом <code>Response</code>, що представляє
          відповідь на запит (не обов'язково вказуючи на успішний HTTP-статус).
        </p>
        <p className="mt-4">
          Для отримання фактичних даних з відповіді необхідно використовувати
          методи об'єкта <code>Response</code>, такі як:
          <ul>
            <li>
              <code>response.json()</code>: Розбирає тіло відповіді як JSON.
              Повертає Promise, який розв'язується з отриманим JSON-об'єктом.
            </li>
            <li>
              <code>response.text()</code>: Розбирає тіло відповіді як текст.
              Повертає Promise, який розв'язується з отриманим текстом.
            </li>
            <li>
              <code>response.blob()</code>: Розбирає тіло відповіді як об'єкт{" "}
              <code>Blob</code> (двійкові дані).
            </li>
            <li>
              <code>response.arrayBuffer()</code>: Розбирає тіло відповіді як
              об'єкт <code>ArrayBuffer</code> (сирі двійкові дані).
            </li>
            <li>
              <code>response.formData()</code>: Розбирає тіло відповіді як
              об'єкт <code>FormData</code> (для роботи з даними форми).
            </li>
          </ul>
        </p>
        <p className="mt-4">
          Об'єкт параметрів функції <code>fetch()</code> може включати такі
          опції, як:
          <ul>
            <li>
              <code>method</code>: HTTP-метод запиту ('GET', 'POST', 'PUT',
              'DELETE' тощо). За замовчуванням - 'GET'.
            </li>
            <li>
              <code>headers</code>: Об'єкт з HTTP-заголовками запиту.
            </li>
            <li>
              <code>body</code>: Тіло запиту (для методів, що підтримують його,
              наприклад, 'POST', 'PUT'). Може бути рядком, об'єктом{" "}
              <code>FormData</code>, <code>Blob</code> тощо.
            </li>
            <li>
              <code>mode</code>: Режим запиту ('cors', 'no-cors',
              'same-origin').
            </li>
            <li>
              <code>credentials</code>: Вказує, чи потрібно надсилати cookies та
              HTTP-аутентифікацію ('omit', 'same-origin', 'include').
            </li>
            <li>
              <code>cache</code>: Вказує політику кешування ('default',
...
inputElement.addEventListener('blur', () => {
  console.log('Інпут втратив фокус.');
    ...
    preview: (
      <div>
        <p>
          Події <code>focus</code> та <code>blur</code> використовуються для
          відстеження стану фокусу елементів на веб-сторінці. Фокус вказує на
          те, який елемент наразі активний та готовий до отримання введення від
          користувача (наприклад, з клавіатури).
        </p>
        <p className="mt-4">
          <ul>
            <li>
              <code>focus</code>: Ця подія відбувається, коли елемент отримує
              фокус. Це може статися, коли користувач клацає на елементі,
              переходить до нього за допомогою клавіші Tab або коли фокус
              встановлюється програмно за допомогою методу <code>focus()</code>.
            </li>
            <li>
              <code>blur</code>: Ця подія відбувається, коли елемент втрачає
              фокус. Це може статися, коли користувач клацає на іншому елементі,
              переходить з елемента за допомогою клавіші Tab або коли фокус
              забирається програмно за допомогою методу <code>blur()</code>.
            </li>
          </ul>
        </p>
        <p className="mt-4">
          ...
          <ul>
            ...
            <li>
              Виконання певних дій при отриманні або втраті фокусу елементом.
            </li>
          </ul>
        </p>
      </div>
  ...
  console.log(i); // Виведе 0, 1, 2, 3, 4
    ...
    preview: (
      <div>
        <p>
          Цикл <code>for</code> є одним з основних керуючих структур у
          програмуванні, що дозволяє багаторазово виконувати блок коду. Він
          складається з трьох необов'язкових виразів, укладених у круглі дужки
          та розділених крапкою з комою:
        </p>
        <pre>
          <code>
        ...
        <ul>
          <li>
            <b>Ініціалізація:</b> Виконується один раз перед початком циклу.
            Зазвичай використовується для оголошення та ініціалізації
            змінної-лічильника.
          </li>
          <li>
            <b>Умова:</b> Перевіряється перед кожною ітерацією циклу. Якщо умова
            є істинною, виконується блок коду. Якщо умова є хибною, цикл
            завершується.
          </li>
          <li>
            <b>Інкремент/декремент:</b> Виконується після кожної ітерації циклу.
            Зазвичай використовується для оновлення змінної-лічильника.
          </li>
        </ul>
        <p className="mt-4">
          Будь-який з цих виразів може бути опущений. Якщо умова опущена, вона
          вважається завжди істинною (створюючи нескінченний цикл, якщо його не
          перервати оператором <code>break</code>).
        </p>
      </div>
    ),
  },
...
for (const key in myObject) {
  console.log(\`Ключ: \${key}, Значення: \${myObject[key]}\`);
    ...
    preview: (
      <div>
        <p>
          Цикл <code>for...in</code> використовується для перебору всіх
          перелічуваних властивостей об'єкта (включаючи успадковані властивості
          з його прототипного ланцюжка). Для кожного ключа (імені властивості) в
          об'єкті виконується вказаний блок коду.
        </p>
        <p className="mt-4">
          <b>Важливо:</b> Цикл <code>for...in</code> не гарантує перебір
          властивостей у якомусь конкретному порядку. Порядок може залежати від
          реалізації JavaScript-рушія.
        </p>
        <p className="mt-4">
          Зазвичай рекомендується використовувати <code>for...in</code> для
          перебору звичайних об'єктів, а не масивів, оскільки порядок індексів у
          масивах може бути непередбачуваним, а також будуть перебиратися
          додаткові перелічувані властивості, якщо вони є. Для перебору
          елементів масиву краще використовувати цикли <code>for</code>,{" "}
          <code>for...of</code> або методи масивів (наприклад,{" "}
          <code>forEach</code>).
        </p>
        <p className="mt-4">
          Для фільтрації лише власних властивостей об'єкта (не успадкованих)
          можна використовувати метод <code>hasOwnProperty()</code>.
        </p>
      </div>
    ),
  },
  {
    title: "for...of",
    description:
      "Перебирає ітеративні об'єкти, повертаючи значення кожного елемента.",
...
for (const char of myString) {
  console.log(char); // Виведе 'П', 'р', 'и', 'в', 'і', 'т'
    ...
    preview: (
      <div>
        <p>
          Цикл <code>for...of</code> використовується для перебору значень
          ітеративних об'єктів, таких як масиви, рядки, Map, Set, об'єкти{" "}
          <code>NodeList</code> та інші об'єкти, що реалізують протокол
          ітератора.
        </p>
        <p className="mt-4">
          На відміну від <code>for...in</code>, <code>for...of</code> перебирає
          саме значення елементів ітеративного об'єкта, а не їхні ключі або
          індекси (хоча для деяких об'єктів, таких як Map, він повертає пари
          [ключ, значення]).
        </p>
        <p className="mt-4">
          <code>for...of</code> є більш зручним та менш схильним до помилок
          способом перебору масивів та інших колекцій порівняно зі стандартним
          циклом <code>for</code> з індексами.
        </p>
      </div>
    ),
  },
...
document.addEventListener('fullscreenchange', () => {
  console.log('Стан повноекранного режиму змінився:', document.fullscreenElement ? 'увімкнено' : 'вимкнено');
    ...
    preview: (
      <div>
        <p>
          Fullscreen API надає інтерфейс для відображення веб-контенту в
          повноекранному режимі, займаючи весь екран користувача.
        </p>
        <p className="mt-4">
          <b>Основні методи та властивості:</b>
          <ul>
            <li>
              <code>element.requestFullscreen()</code>: Запитує перехід елемента
              в повноекранний режим. Повертає Promise.
            </li>
            <li>
              <code>document.exitFullscreen()</code>: Запитує вихід з
              повноекранного режиму.
            </li>
            <li>
              <code>document.fullscreenElement</code>: Повертає елемент, який
              зараз відображається в повноекранному режимі, або{" "}
              <code>null</code>, якщо повноекранний режим неактивний.
            </li>
            <li>
              <code>document.fullscreenEnabled</code>: Повертає булеве значення,
              що вказує, чи дозволено використання повноекранного режиму.
            </li>
            <li>
              <code>document.onfullscreenchange</code>: Обробник події, що
              викликається при зміні стану повноекранного режиму.
            </li>
            <li>
              <code>document.onfullscreenerror</code>: Обробник події, що
              викликається при виникненні помилки під час спроби переходу в
              повноекранний режим.
            </li>
          </ul>
        </p>
        <p className="mt-4">
          З міркувань безпеки, зазвичай перехід у повноекранний режим може бути
          ініційовано лише у відповідь на безпосередню дію користувача
          (наприклад, клік миші або натискання клавіші).
        </p>
      </div>
    ),
  },
  {
    title: "function",
    description: "Виконує набір інструкцій, які можна викликати багаторазово.",
    code: `function greet(name) {
  console.log('Hello, ' + name + '!');
}

greet('Alex'); // Виведе: Hello, Alex!`,
    preview: (
      <div>
        <p>
          Функція дозволяє організувати багаторазовий виклик певного набору
          інструкцій. Ви можете передавати аргументи та отримувати значення у
          вигляді результату виконання.
        </p>
        <p className="mt-4">
          <b>Основні моменти:</b>
          <ul>
            <li>
              <code>function functionName()</code>: Оголошення функції.
            </li>
            <li>
              <code>arguments</code>: Передача параметрів у функцію.
            </li>
            <li>
              <code>return</code>: Повернення значень з функції.
            </li>
          </ul>
        </p>
      </div>
  ...
  },
  {
    title: "Geolocation",
    description:
      "Дозволяє отримувати географічне положення користувача пристрою.",
    code: `navigator.geolocation.getCurrentPosition((position) => {
  console.log('Широта:', position.coords.latitude);
  console.log('Довгота:', position.coords.longitude);
}, (error) => {
  console.log('Помилка отримання позиції:', error);
});`,
    preview: (
      <div>
        <p>
          Geolocation API дозволяє отримати географічне місцезнаходження
          користувача. Це зручно для додатків, які потребують інформації про
          місцезнаходження.
        </p>
        <p className="mt-4">
          <b>Основні методи та властивості:</b>
          <ul>
            <li>
              <code>getCurrentPosition()</code>: Отримує поточне
              місцезнаходження.
            </li>
            <li>
              <code>watchPosition()</code>: Постійно відслідковує зміни в
              місцезнаходженні.
            </li>
            <li>
              <code>position.coords</code>: Доступ до координат
              місцезнаходження.
            </li>
          </ul>
        </p>
  ...
  },
  {
    title: "Global",
    description:
      "Різні середовища виконання можуть надавати власні глобальні об'єкти (у браузері — window, в Node.js — global).",
    code: `console.log(window); // У браузері
console.log(global); // У Node.js`,
    preview: (
      <div>
        <p>
          Глобальні об'єкти надають доступ до основних функцій середовища
          виконання, таких як доступ до вікна браузера або об'єкта глобального
          середовища в Node.js.
        </p>
        <p className="mt-4">
          <b>Основні моменти:</b>
          <ul>
            <li>
              <code>window</code>: Глобальний об'єкт для браузера.
            </li>
            <li>
              <code>global</code>: Глобальний об'єкт для Node.js.
            </li>
          </ul>
        </p>
      </div>
  ...
  },
  {
    title: "HashChange",
    description:
      "Відстежує зміни в частині URL після символу # без перезавантаження сторінки.",
    code: `window.addEventListener('hashchange', () => {
  console.log('URL змінився:', window.location.hash);
});`,
    preview: (
      <div>
        <p>
          Подія <code>hashchange</code> спрацьовує, коли змінюється частина URL
          після символу <code>#</code>, що дозволяє додавати хеші для навігації
          на сторінці без перезавантаження.
        </p>
        <p className="mt-4">
          <b>Основні моменти:</b>
          <ul>
            <li>
              <code>window.location.hash</code>: Отримує або задає хеш URL.
            </li>
            <li>
              <code>hashchange</code>: Подія, яка спрацьовує при зміні хеша в
              URL.
            </li>
          </ul>
        </p>
  ...
  },
  {
    title: "History",
    description:
      "Дозволяє взаємодіяти з історією сесії браузера, даваючи можливість навігації вперед, назад та у інші місця в історії сторінки.",
    code: `window.history.pushState({}, '', '/newPage');
window.history.back(); // Перехід до попередньої сторінки`,
    preview: (
      <div>
        <p>
          Історія сесії дозволяє маніпулювати історією перегляду сторінок без
          перезавантаження, даючи змогу додавати нові записи або переходити між
          ними.
        </p>
        <p className="mt-4">
          <b>Основні методи:</b>
          <ul>
            <li>
              <code>pushState()</code>: Додає новий запис в історію без
              перезавантаження сторінки.
            </li>
            <li>
              <code>back()</code>: Повертає на попередню сторінку в історії.
            </li>
            <li>
              <code>forward()</code>: Переходить на наступну сторінку в історії.
            </li>
          </ul>
        </p>
      </div>
  ...
  },
  {
    ...
    description:
      "Представляє колекцію елементів DOM, які можна адресувати за індексом або ім'ям.",
    code: `const elements = document.getElementsByTagName('p');
console.log(elements[0]); // Перший <p> елемент`,
    preview: (
      <div>
        <p>
          <code>HTMLCollection</code> є живою колекцією елементів, що
          відповідають певному селектору. Колекція автоматично оновлюється при
          зміні DOM.
        </p>
        <p className="mt-4">
          <b>Основні моменти:</b>
          <ul>
            <li>
              <code>getElementsByTagName()</code>: Повертає{" "}
              <code>HTMLCollection</code> для всіх елементів з певним тегом.
            </li>
            <li>
              <code>getElementsByClassName()</code>: Повертає{" "}
              <code>HTMLCollection</code> для всіх елементів з певним класом.
            </li>
            <li>
              <code>length</code>: Властивість, що повертає кількість елементів
              в колекції.
            </li>
          </ul>
        </p>
  ...
  {
    title: "if...else",
    description: "Виконує різний код залежно від умови.",
    code: `const age = 18;

if (age >= 18) {
  console.log('Ви є дорослим!');
} else {
  console.log('Ви не є дорослим!');
}`,
    preview: (
      <div>
        <p>
          Оператор <code>if...else</code> дозволяє виконати одну частину коду,
          якщо умова є істинною, і іншу — якщо умова хибна.
        </p>
        <p className="mt-4">
          <b>Основні моменти:</b>
          <ul>
            <li>
              <code>if</code>: Перевіряє умову і виконує код, якщо вона істинна.
            </li>
            <li>
              <code>else</code>: Виконується, якщо умова у <code>if</code>{" "}
              хибна.
            </li>
            <li>
              <code>else if</code>: Для перевірки кількох умов послідовно.
            </li>
          </ul>
        </p>
      </div>
  ...
  },
  {
    title: "Input",
    description:
      "Відстежує зміни у введених користувачем даних в текстових полях форми.",
    code: `const inputField = document.getElementById('inputField');
...
inputField.addEventListener('input', (event) => {
  console.log('Введено:', event.target.value);
    ...
    preview: (
      <div>
        <p>
          Подія <code>input</code> спрацьовує при кожній зміні введених даних у
          текстовому полі форми. Це дозволяє відстежувати введення користувача в
          реальному часі.
        </p>
        <p className="mt-4">
          <b>Основні моменти:</b>
          <ul>
            <li>
              <code>addEventListener('input', callback)</code>: Відстежує зміни
              введення в текстовому полі.
            </li>
            <li>
              <code>event.target.value</code>: Отримує поточне значення
              текстового поля.
            </li>
          </ul>
        </p>
  ...
  },
  {
    title: "JSON",
    description:
      "Містить статичні методи для розбору значень з формату JSON та конвертації значень до цього формату.",
    code: `const jsonString = '{"name": "Alex", "age": 30}';
const parsedObject = JSON.parse(jsonString);
console.log(parsedObject);

const newObject = { name: 'John', age: 25 };
const jsonStringified = JSON.stringify(newObject);
console.log(jsonStringified);`,
    preview: (
      <div>
        <p>
          <code>JSON</code> надає два основні методи для роботи з форматом JSON:{" "}
          <code>parse()</code> для перетворення JSON-стрічки в об'єкт і{" "}
          <code>stringify()</code> для перетворення об'єкта в JSON-стрічку.
        </p>
        <p className="mt-4">
          <b>Основні методи:</b>
          <ul>
            <li>
              <code>JSON.parse()</code>: Перетворює JSON-стрічку в
              JavaScript-об'єкт.
            </li>
            <li>
              <code>JSON.stringify()</code>: Перетворює JavaScript-об'єкт в
              JSON-стрічку.
            </li>
          </ul>
        </p>
  ...
  },
  {
    title: "Keyboard",
    description:
      "Відстежує натискання та відпускання клавіш на клавіатурі користувача.",
...
document.addEventListener('keyup', (event) => {
  console.log('Відпущена клавіша:', event.key);
    ...
    preview: (
      <div>
        <p>
          Події <code>keydown</code> і <code>keyup</code> дозволяють
          відстежувати натискання та відпускання клавіш на клавіатурі.
        </p>
        <p className="mt-4">
          <b>Основні моменти:</b>
          <ul>
            <li>
              <code>keydown</code>: Подія, що спрацьовує при натисканні клавіші.
            </li>
            <li>
              <code>keyup</code>: Подія, що спрацьовує при відпусканні клавіші.
            </li>
            <li>
              <code>event.key</code>: Властивість, що містить символ клавіші.
            </li>
          </ul>
        </p>
      </div>
  ...
  {
    title: "let",
    description: "Дозволяє оголосити змінну з блочною областю видимості.",
    code: `let counter = 0;

if (true) {
  let counter = 5;
  console.log(counter); // Виведе 5
}

console.log(counter); // Виведе 0`,
    preview: (
      <div>
        <p>
          <code>let</code> дозволяє оголошувати змінні з блочною областю
          видимості, що означає, що вони існують тільки в межах блоку, у якому
          оголошені.
        </p>
        <p className="mt-4">
          <b>Основні моменти:</b>
          <ul>
            <li>
              <code>let</code>: Змінна має область видимості, обмежену блоком, у
  ...
  },
  {
    title: "Location",
    description:
      "Представляє інформацію про поточний URL документа та надає методи для зміни цього URL.",
    code: `console.log(window.location.href); // Показує поточний URL

window.location.href = 'https://example.com'; // Перенаправляє на інший URL`,
    preview: (
      <div>
        <p>
          <code>window.location</code> надає доступ до поточного URL та дозволяє
          змінювати його для навігації на інші сторінки.
        </p>
        <p className="mt-4">
          <b>Основні властивості та методи:</b>
          <ul>
            <li>
              <code>window.location.href</code>: Повертає або задає поточний
              URL.
            </li>
            <li>
              <code>window.location.assign()</code>: Перенаправляє на іншу
              сторінку.
            </li>
            <li>
              <code>window.location.reload()</code>: Перезавантажує сторінку.
            </li>
          </ul>
        </p>
      </div>
  ...
  },
  {
    title: "Math",
    description:
      "Містить статичні властивості та методи для математичних констант і функцій.",
    code: `console.log(Math.PI); // Виведе: 3.141592653589793
console.log(Math.sqrt(16)); // Виведе: 4`,
    preview: (
      <div>
        <p>
          <code>Math</code> містить числові константи та методи для математичних
          операцій, таких як обчислення кореня, округлення, отримання випадкових
          чисел тощо.
        </p>
        <p className="mt-4">
          <b>Основні методи:</b>
          <ul>
            ...
            <li>
              <code>Math.sqrt()</code>: Обчислює квадратний корінь з числа.
            </li>
            <li>
              <code>Math.random()</code>: Повертає випадкове число між 0 і 1.
            </li>
          </ul>
        </p>
      </div>
  ...
  },
  {
    ...
    description:
      "Дозволяє визначати та відслідковувати зміни в результаті запитів медіа (наприклад, зміни у розмірі екрана або орієнтації пристрою).",
    code: `const mediaQuery = window.matchMedia('(max-width: 600px)');

mediaQuery.addEventListener('change', (event) => {
  if (event.matches) {
    console.log('Екран став менше 600px');
  } else {
    console.log('Екран більше 600px');
  }
});`,
    preview: (
      <div>
        <p>
          <code>MediaQueryList</code> дозволяє працювати з медіа-запитами і
          реагувати на зміни умов перегляду, такі як зміна ширини вікна
          браузера.
        </p>
        <p className="mt-4">
          <b>Основні моменти:</b>
          <ul>
            <li>
              <code>matchMedia()</code>: Створює об'єкт для медіа-запиту.
            </li>
            <li>
              <code>addEventListener('change')</code>: Реагує на зміни в
              результатах медіа-запиту.
            </li>
          </ul>
        </p>
      </div>
  ...
  },
  {
    title: "Mouse",
    description:
      "Дозволяє обробляти взаємодії користувача з мишею, як-от кліки чи наведення.",
...
button.addEventListener('mouseover', () => {
  console.log('Мишка над кнопкою!');
});`,
    preview: (
      <div>
        <p>
          Події миші дозволяють відслідковувати взаємодії користувача з
          елементами, такі як кліки, наведення, відведення та інші.
        </p>
        <p className="mt-4">
          <b>Основні події:</b>
          <ul>
            <li>
              <code>click</code>: Спрацьовує при натисканні кнопки миші.
            </li>
            <li>
              <code>mouseover</code>: Спрацьовує, коли курсор миші наводиться на
              елемент.
            </li>
            <li>
              <code>mouseout</code>: Спрацьовує, коли курсор миші залишає
              елемент.
            </li>
          </ul>
        </p>
  ...
  },
  {
    title: "Navigator",
    description:
      "Надає інформацію про браузер користувача та систему, а також містить деякі методи для контролю певних функцій браузера.",
    code: `console.log(navigator.userAgent); // Показує інфо про браузер
console.log(navigator.language); // Показує мову користувача`,
    preview: (
      <div>
        <p>
          <code>navigator</code> надає доступ до інформації про браузер,
          операційну систему та інші параметри середовища користувача.
        </p>
        <p className="mt-4">
          <b>Основні властивості:</b>
          <ul>
            <li>
              <code>navigator.userAgent</code>: Повертає рядок, що містить
              інформацію про браузер та операційну систему.
            </li>
            <li>
              <code>navigator.language</code>: Повертає мову користувача.
            </li>
          </ul>
        </p>
      </div>
  ...
  },
  {
    title: "NodeList",
    description:
      "Представляє собою колекцію вузлів і дозволяє перебирати та маніпулювати цими вузлами.",
    code: `const listItems = document.querySelectorAll('li');
...
listItems.forEach((item) => {
  console.log(item.textContent);
    ...
    preview: (
      <div>
        <p>
          <code>NodeList</code> є колекцією вузлів DOM, з якими можна працювати,
          як із масивом. Часто використовується з методами як-от{" "}
        ...
        <p className="mt-4">
          ...
          <ul>
            <li>
              <code>forEach()</code>: Метод для перебору елементів у NodeList.
            </li>
          </ul>
        </p>
      </div>
  ...
  },
  {
    title: "Number",
    description:
      "Є обгорткою над числовим значенням та містить константи та методи для роботи з числами.",
    code: `console.log(Number.isInteger(4)); // true
console.log(Number.parseFloat('3.14')); // 3.14`,
    preview: (
      <div>
        <p>
          <code>Number</code> — це вбудований об'єкт, який містить методи для
          перевірки та маніпуляції з числами.
        </p>
        <p className="mt-4">
          <b>Основні методи:</b>
          <ul>
            <li>
              <code>Number.isInteger()</code>: Перевіряє, чи є число цілим.
            </li>
            <li>
              <code>Number.parseFloat()</code>: Перетворює рядок на число з
              плаваючою точкою.
            </li>
          </ul>
        </p>
  ...
  {
  title: "Object",
  description: "Є одним з типів даних у JavaScript й використовується для зберігання різноманітних колекцій з ключами.",
  code: `const машина = {
  марка: "Toyota",
  рік: 2020,
  сигнал() {
...
машина.сигнал();`,
  preview: (
    <div>
      <p>Об'єкти дозволяють зберігати зв'язані дані та/або функціональність (методи) в одному місці.</p>
      <ul className="mt-4 list-disc list-inside">
        <li><code>{`об'єкт.властивість`}</code> — доступ до значення.</li>
        <li><code>Object.keys(obj)</code> — отримати всі ключі об'єкта.</li>
        <li><code>Object.values(obj)</code> — отримати всі значення.</li>
      </ul>
    </div>
  ),
},
{
  title: "Operators",
  description: "Оператори виконують дії над значеннями: арифметичні, логічні, порівняння, присвоєння та інші.",
  code: `let x = 5;
let y = 2;
let результат = x + y; // 7`,
  preview: (
    <ul className="list-disc list-inside">
      <li><b>Арифметичні</b>: <code>+ - * / %</code></li>
      <li><b>Порівняння</b>: <code>== === != !== </code></li>
      <li><b>Логічні</b>: <code>&& || !</code></li>
      <li><b>Присвоєння</b>: <code>= += -= *=</code> тощо</li>
      <li><b>Унарні</b>: <code>typeof, delete, void</code></li>
    </ul>
  ),
},
{
  title: "PageTransition",
  description: "Контролює події завантаження або вивантаження сторінки, включаючи переходи між ними.",
  code: `window.addEventListener("pageshow", (event) => {
  console.log("Сторінка відображена:", event.persisted);
});

window.addEventListener("pagehide", (event) => {
  console.log("Сторінка ховається:", event.persisted);
});`,
  preview: (
    <p>
      Події <code>pageshow</code> та <code>pagehide</code> корисні для роботи з кешованими сторінками, такими як BFCache.
    </p>
  ),
},
{
  title: "PopState",
  description: "Дозволяє реагувати на зміни історії браузера при навігації вперед або назад.",
  code: `window.addEventListener("popstate", (event) => {
  console.log("Стан змінено:", event.state);
});`,
  preview: (
    <p>
      Подія <code>popstate</code> викликається, коли користувач переходить назад або вперед у історії.
    </p>
  ),
},
{
  title: "Precedence",
  description: "Порядок, у якому оператори виконуються в складних виразах.",
  code: `let результат = 2 + 3 * 4; // 14, бо * має вищий пріоритет за +`,
  preview: (
    <p>
      Важливо знати пріоритет операторів, щоб уникати неочікуваних результатів. Дужки можуть змінювати пріоритет:
      <br />
      <code>(2 + 3) * 4 = 20</code>
    </p>
  ),
...
{
  title: "Progress",
  description: "Дозволяє відстежувати прогрес завантаження ресурсів або обробки даних у реальному часі.",
  code: `xhr.onprogress = function(event) {
  if (event.lengthComputable) {
    console.log(\`Завантажено: \${event.loaded} з \${event.total}\`);
  }
};`,
  preview: (
    <p>
      Подія <code>progress</code> використовується, наприклад, з <code>XMLHttpRequest</code> або <code>Fetch</code> API для створення індикаторів завантаження.
    </p>
  ),
},
{
  title: "RegExp",
  description: "Використовується для пошуку тексту за певним шаблоном.",
  code: `const текст = "Привіт, світ!";
const патерн = /світ/;
console.log(патерн.test(текст)); // true`,
  preview: (
    <p>
      Регулярні вирази — потужний інструмент для пошуку і валідації рядків. Наприклад:
      <br />
      <code>/\\d+/</code> — знайде одну або більше цифр
    </p>
  ),
},
{
  title: "return",
  description: "Припиняє виконання функції та повертає вказане значення.",
  code: `function добуток(a, b) {
  return a * b;
}
console.log(добуток(3, 4)); // 12`,
  preview: (
    <p>
      <code>return</code> зупиняє виконання функції та повертає результат виклику.
    </p>
  ),
},
{
  title: "Screen",
  description: "Надає інформацію про розмір екрану користувача та його характеристики.",
  code: `console.log("Ширина екрану:", screen.width);
console.log("Висота екрану:", screen.height);`,
  preview: (
    <p>
      Об'єкт <code>screen</code> дає доступ до фізичних параметрів екрана: ширина, висота, доступна область, глибина кольору тощо.
    </p>
  ),
}
];
